...
const Lycee = () => {
  ...
  const FILE_TYPES = useMemo(() => ({
    ...
    EXAMNATIONAL: { name: 'examnational', icon: <FaFileAlt className="text-purple" />, color: '#9C27B0' },
    CONCOURS: { name: 'concours', icon: <FaFileAlt className="text-info" />, color: '#00BCD4' },
  ...
  const fetchFilieres = useCallback(async (classId) => {
    ...
    try {
      setState(prev => ({ 
        ...prev, 
        loadingStates: { ...prev.loadingStates, filieres: true },
        error: null 
      }));
      ...
      setState(prev => ({
        ...
        loadingStates: { ...prev.loadingStates, filieres: false }
      }));
    } catch (error) {
      setState(prev => ({
        ...prev,
        loadingStates: { ...prev.loadingStates, filieres: false },
        error: error.response?.data?.message || "Erreur de chargement des filières"
      }));
    }
  }, [API_URL]);

  const fetchSubjects = useCallback(async (classId, filiereId) => {
    if (!classId || !filiereId) return;
  ...
  const fetchFiles = useCallback(
    debounce(async (classId, filiereId, subject) => {
      if (!classId || !filiereId || !subject) {
      ...
      try {
        ...
        (response.data?.data || []).forEach(file => {
          ...
          if (parseInt(file.filiere_id) !== parseInt(filiereId)) return;
  ...
  useEffect(() => {
    if (selections.classId) {
      fetchFilieres(selections.classId);
      setSelections(prev => ({ ...prev, filiereId: '', subject: '' }));
    }
  }, [selections.classId, fetchFilieres]);

  useEffect(() => {
    if (selections.classId && selections.filiereId) {
      fetchSubjects(selections.classId, selections.filiereId);
      setSelections(prev => ({ ...prev, subject: '' }));
    }
  }, [selections.classId, selections.filiereId, fetchSubjects]);

  useEffect(() => {
    fetchFiles(selections.classId, selections.filiereId, selections.subject);
    return () => fetchFiles.cancel();
  }, [selections.classId, selections.filiereId, selections.subject, fetchFiles]);
  ...
  const Selectors = React.memo(() => (
    <div className="selector-container">
      <Row>
        ...
        <Col md={4} className="mb-3">
          <Form.Group controlId="filiere-select">
            <Form.Label className="selector-label">Branche :</Form.Label>
            <Form.Select
              value={selections.filiereId}
              onChange={handleFiliereChange}
              disabled={!selections.classId || state.loadingStates.filieres}
              className="selector-input"
            >
        ...
        <Col md={4} className="mb-3">
          <Form.Group controlId="subject-select">
            ...
            <Form.Select
              ...
              disabled={!selections.filiereId || state.loadingStates.subjects}
              className="selector-input"
            >
  ...
  return (
    ...
    >
      ...
      <Container className="py-4">
        ...
        >
          <div className="main-container">
            ...
            <p className="page-subtitle text-center">
              Découvrez toutes les ressources pédagogiques pour le niveau lycée
            </p>
          </div>
        </motion.div>
        ...
        {selections.classId && selections.filiereId && selections.subject && (
          <>
            <SearchAndFilter />
            <SemesterTabs />
          </>
        ...
        {!state.loadingStates.files && !state.error && selections.classId && selections.filiereId && selections.subject && (
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            transition={{ delay: 0.2 }}
